...
describe('Tests d\'Intégration - API de Taux de Change', () => {
    ...
    describe('Intégration avec API externe mockée', () => {
        ...
        test('devrait gérer une API en panne', async () => {
            ...
            await expect(mockAPI.getRate('EUR', 'USD'))
                .rejects.toThrow('API temporarily unavailable');
        });

        test('devrait gérer une devise non supportée par l\'API', async () => {
            await expect(mockAPI.getRate('EUR', 'JPY'))
                .rejects.toThrow('Exchange rate not found for EUR-JPY');
        });
    });
    ...
    describe('Service de conversion avec fallback', () => {
        ...
        class ExtendedConversionService extends ConversionService {
            ...
            async convertWithExternalAPI(from, to, amount) {
                try {
                    ...
                    const convertedAmount = parseFloat((amount * apiResult.rate).toFixed(2));
                    ...
                    console.log('External API failed, using internal rates:', error.message);
        ...
        test('devrait utiliser l\'API externe quand elle est disponible', async () => {
            ...
            const result = await extendedService.convertWithExternalAPI('EUR', 'USD', 100);
        ...
        test('devrait gérer le timeout de l\'API externe', async () => {
            ...
            const conversionPromise = extendedService.convertWithExternalAPI('EUR', 'USD', 100);
            
            await expect(Promise.race([conversionPromise, timeoutPromise]))
                .rejects.toThrow('API timeout');
        }, 10000);
    });    describe('Tests de charge avec API mockée', () => {        test('devrait gérer plusieurs appels simultanés', async () => {
        ...
        test('devrait gérer les échecs partiels d\'API', async () => {
            ...
            mockAPI.getRate = async function(from, to) {
                ...
                mockAPI.getRate('EUR', 'USD').catch(e => ({ error: e.message }))
            );
