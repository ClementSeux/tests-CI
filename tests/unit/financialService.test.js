...
describe("FinancialService - Tests Unitaires", () => {
    ...
    describe("calculateTTC - Validation des erreurs", () => {
        test("devrait lever une erreur pour des paramètres manquants", () => {
            expect(() => financialService.calculateTTC()).toThrow(
                "Paramètres manquants: ht et taux sont requis"
            );
            expect(() => financialService.calculateTTC(100)).toThrow(
                "Paramètres manquants: ht et taux sont requis"
            );
            expect(() => financialService.calculateTTC(null, 20)).toThrow(
                "Paramètres manquants: ht et taux sont requis"
            );
        });

        test("devrait lever une erreur pour des valeurs invalides", () => {
            expect(() => financialService.calculateTTC("invalid", 20)).toThrow(
                "HT et taux doivent être des nombres valides"
            );
            expect(() => financialService.calculateTTC(100, "invalid")).toThrow(
                "HT et taux doivent être des nombres valides"
            );
            expect(() => financialService.calculateTTC(NaN, 20)).toThrow(
                "HT et taux doivent être des nombres valides"
            );
        });

        test("devrait lever une erreur pour HT négatif", () => {
            expect(() => financialService.calculateTTC(-10, 20)).toThrow(
                "Le montant HT ne peut pas être négatif"
            );
        });

        test("devrait lever une erreur pour taux de TVA invalide", () => {
            expect(() => financialService.calculateTTC(100, -5)).toThrow(
                "Le taux de TVA doit être entre 0 et 100"
            );
            expect(() => financialService.calculateTTC(100, 101)).toThrow(
                "Le taux de TVA doit être entre 0 et 100"
            );
        });
    });
    ...
    describe("calculateDiscount - Validation des erreurs", () => {
        test("devrait lever une erreur pour des paramètres manquants", () => {
            expect(() => financialService.calculateDiscount()).toThrow(
                "Paramètres manquants: prix et pourcentage sont requis"
            );
            expect(() => financialService.calculateDiscount(100)).toThrow(
                "Paramètres manquants: prix et pourcentage sont requis"
            );
            expect(() => financialService.calculateDiscount(null, 10)).toThrow(
                "Paramètres manquants: prix et pourcentage sont requis"
            );
        });
        ...
        test("devrait lever une erreur pour des valeurs invalides", () => {
            expect(() =>
                financialService.calculateDiscount("invalid", 10)
            ).toThrow("Prix et pourcentage doivent être des nombres valides");
            expect(() =>
                financialService.calculateDiscount(100, "invalid")
            ).toThrow("Prix et pourcentage doivent être des nombres valides");
            expect(() => financialService.calculateDiscount(NaN, 10)).toThrow(
                "Prix et pourcentage doivent être des nombres valides"
            );
        });

        test("devrait lever une erreur pour prix négatif", () => {
            expect(() => financialService.calculateDiscount(-10, 10)).toThrow(
                "Le prix ne peut pas être négatif"
            );
        });

        test("devrait lever une erreur pour pourcentage invalide", () => {
            expect(() => financialService.calculateDiscount(100, -5)).toThrow(
                "Le pourcentage de remise doit être entre 0 et 100"
            );
            expect(() => financialService.calculateDiscount(100, 101)).toThrow(
                "Le pourcentage de remise doit être entre 0 et 100"
            );
        });
    });
