...
describe("ConversionService - Tests Unitaires", () => {
    ...
    describe("Validation des erreurs", () => {
        test("devrait lever une erreur pour des paramètres manquants", () => {
            expect(() => conversionService.convert()).toThrow(
                "Paramètres manquants: from, to et amount sont requis"
            );
            expect(() => conversionService.convert("EUR")).toThrow(
                "Paramètres manquants: from, to et amount sont requis"
            );
            expect(() => conversionService.convert("EUR", "USD")).toThrow(
                "Paramètres manquants: from, to et amount sont requis"
            );
        });
        ...
        test("devrait lever une erreur pour un montant invalide", () => {
            expect(() =>
                conversionService.convert("EUR", "USD", "invalid")
            ).toThrow("Le montant doit être un nombre positif");
            expect(() => conversionService.convert("EUR", "USD", NaN)).toThrow(
                "Le montant doit être un nombre positif"
            );
            expect(() => conversionService.convert("EUR", "USD", -10)).toThrow(
                "Le montant doit être un nombre positif"
            );
        });

        test("devrait lever une erreur pour une conversion non supportée", () => {
            expect(() => conversionService.convert("EUR", "JPY", 100)).toThrow(
                "Conversion non supportée: EUR vers JPY"
            );
            expect(() => conversionService.convert("ABC", "XYZ", 100)).toThrow(
                "Conversion non supportée: ABC vers XYZ"
            );
        });
    ...
    describe("fetchExchangeRateFromAPI", () => {
        test("devrait simuler un appel API réussi", async () => {
            const rate = await conversionService.fetchExchangeRateFromAPI(
                "EUR",
                "USD"
            );
        ...
        test("devrait simuler un appel API échoué", async () => {
            await expect(
                conversionService.fetchExchangeRateFromAPI("EUR", "JPY")
